...
export function AuthLayout({ children }: AuthLayoutProps) {
  return (
    <div className="min-h-screen relative">
      ...
      <motion.div
        className="fixed bottom-8 left-8 text-xs text-gray-500 font-bold uppercase tracking-widest backdrop-blur-sm bg-black/30 px-3 py-2 rounded-lg border border-cyan-500/20"
      ...
      <motion.div
        className="fixed top-8 right-8 text-xs text-gray-500 font-bold uppercase tracking-widest backdrop-blur-sm bg-black/30 px-3 py-2 rounded-lg border border-cyan-500/20"
        initial={{ opacity: 0, x: 20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 1 }}
      >
